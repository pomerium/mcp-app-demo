...
export function ServerSettings() {
  ...
  const addServer = (e: React.FormEvent) => {
    ...
    if (newServer.name && newServer.url) {
      ...
      showNotification('Server Added', `${newServer.name} has been added to your servers list.`);
  ...
  const connectToServer = async (serverId: string) => {
    ...
    try {
      ...
      showNotification('Connection Failed', 'Unable to connect to the server. Please try again.');
    }
  };
  ...
  const ServerForm = () => (
    <form onSubmit={addServer} className="space-y-4">
      <div className="space-y-2">
        <Label htmlFor="name" className="text-gray-900 dark:text-gray-100">Server Name</Label>
        <input
          id="name"
          type="text"
      ...
      <div className="space-y-2">
        <Label htmlFor="url" className="text-gray-900 dark:text-gray-100">Server URL</Label>
        <input
          id="url"
          type="url"
          value={newServer.url}
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300">{server.url}</p>
                </div>
                <Button
                  onClick={() => connectToServer(server.id)}
                  disabled={server.status === 'connecting' || server.status === 'connected'}
                  variant={server.status === 'connected' ? 'outline' : 'primary'}
