...
const fetchJobStatus = async (requestId: string): Promise<BackgroundJobStatusResponse> => {
  ...
  if (!response.ok) {
    throw new HttpError(response.status, `Failed to fetch job status: ${response.status}`)
  }
...
export const useBackgroundJobStatus = (
  ...
  return useQuery({
    queryKey: ['backgroundJobStatus', job?.requestId],
    queryFn: () => {
      if (!job?.requestId) {
        throw new Error('No request ID available')
      }
      return fetchJobStatus(job.requestId)
    },
    enabled: !!job?.requestId && job.status === 'running', // Only poll running jobs with valid requestId
    refetchInterval, // Use the provided refetchInterval
